...
int main() {
	...
	ModelAnim vampiroidle("resources/models/Vampiro/vampiro.dae");
	vampiroidle.initShaders(animShader.ID);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		modelOp = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, 30.0f)); // translate it down so it's at the center of the scene
		//modelOp = glm::scale(modelOp, glm::vec3(0.05f));	// it's a bit too big for our scene, so scale it down
		//modelOp = glm::rotate(modelOp, glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		animShader.setMat4("model", modelOp);
		vampiroidle.Draw(animShader);
